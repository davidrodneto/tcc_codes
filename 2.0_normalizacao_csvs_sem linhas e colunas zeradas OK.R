...
remove_accents <- function(string) {
  #stri_trans_general(string, "Hangul-Latin")
...
generate_matrices <- function(json_interactions) {
  ...
  for (row in 1:length(json_interactions$data$jogadores[[1]])) {
    ...
    if (name_counts[normalized_name] > 1) {
      #final_name <- paste(player_name, player_id, sep="_")
  ...
  for (row in 1:length(json_interactions$data$jogadores[[1]])) {
    ...
    for (row_interaction in 1:length(json_interactions$data$jogadores[[4]][[row]][[1]])) {
      ...
      normalized_interaction_name <- ifelse(
        ...
        #paste(interaction_player_name, interaction_player_id, sep="_"), 
      ...
      #if (normalized_player_name %in% rownames(df_a) && normalized_interaction_name %in% colnames(df_a)) {
      #  df_a[normalized_player_name, normalized_interaction_name] <- df_a[normalized_player_name, normalized_interaction_name] + quantity_player_name
      #}
      
      #if (normalized_player_name %in% rownames(df_b) && normalized_interaction_name %in% colnames(df_b)) {
      #  df_b[normalized_player_name, normalized_interaction_name] <- df_b[normalized_player_name, normalized_interaction_name] + quantity_player_name
      #}
    }
  }
...
# Execução principal
main <- function() {
  create_dir("C:/Users/USER/Desktop/csvs_normalizadosdec3")
  ...
  for (match in match_info$all_matches) {
    ...
    if (!is.null(json_interactions)) {
      ...
      write.table(transition_a, file = sprintf("C:/Users/USER/Desktop/csvs_normalizadosdec3/%s_%s_%s_%s_normalizado.csv", match$round, match$id, key_a, team_a_name), sep = ";", row.names = TRUE, col.names = NA, fileEncoding = 'UTF-8')
      ...
      write.table(transition_b, file = sprintf("C:/Users/USER/Desktop/csvs_normalizadosdec3/%s_%s_%s_%s_normalizado.csv", match$round, match$id, key_b, team_b_name), sep = ";", row.names = TRUE, col.names = NA, fileEncoding = 'UTF-8')
      
      #write.table(df_a, file = sprintf("C:/Users/USER/Desktop/csvs/%s_%s_%s_%s.csv", match$round, match$id, key_a, team_a_name), sep = ";", row.names = TRUE, col.names = NA, fileEncoding = 'latin1')
      #write.table(transition_a, file = sprintf("C:/Users/USER/Desktop/csvs_normalizadosdec3/%s_%s_%s_%s_normalizado.csv", match$round, match$id, key_a, team_a_name), sep = ";", row.names = TRUE, col.names = NA, fileEncoding = 'latin1')
      
      #write.table(df_b, file = sprintf("C:/Users/USER/Desktop/csvs/%s_%s_%s_%s.csv", match$round, match$id, key_b, team_b_name), sep = ";", row.names = TRUE, col.names = NA, fileEncoding = 'latin1')
      #write.table(transition_b, file = sprintf("C:/Users/USER/Desktop/csvs_normalizadosdec3/%s_%s_%s_%s_normalizado.csv", match$round, match$id, key_b, team_b_name), sep = ";", row.names = TRUE, col.names = NA, fileEncoding = 'latin1')
    }
  }
