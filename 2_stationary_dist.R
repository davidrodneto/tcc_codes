...
### Verificar e corrigir somas das linhas
  ...
  if (is_irreducible_k_steps(normalized_matrix)) {
    ...
    fluent_player_file <- file.path(output_dir, sub("\\.csv$", "_jogador_mais_influente.txt", basename(file_name)))
    write(paste("Jogador mais influente:", fluente_jogador), fluent_player_file)
